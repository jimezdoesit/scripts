...
coloredEcho "Enter password for $user!" green

case ${logging} in
   y|Y) pssh -i -t 0 -h hostnames -e log -l ${user} -A "${cmd_exec}" 2>&1 | tee logfile ;;
   n|N) pssh -i -t 0 -h hostnames -e log -l ${user} -A "${cmd_exec}" ;;
